...
<form action="{{ route('profile.update-profil', $profil->id) }}" method="POST">
    @csrf
    @method('PATCH')
    ...
    <div class="form-group mb-3">
        ...
        <input type="text" class="form-control" id="section" name="section" value="{{ $profil->section }}" required>
        <x-input-error class="mt-2 text-danger" :messages="$errors->get('section')" />
    </div>
    <div class="form-group mb-3">
        <label for="position">Position</label>
        <input type="text" class="form-control" id="position" name="position" value="{{ $profil->position }}" required>
        <x-input-error class="mt-2 text-danger" :messages="$errors->get('position')" />
    </div>
    <div class="form-group mb-3">
        <label for="join_date">Join Date</label>
        <input type="date" class="form-control" id="join_date" name="join_date" value="{{ $profil->join_date }}" required>
